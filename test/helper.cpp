...
std::string buildRelease(std::string defines) {
	return std::string("gcc -O3 -DNDEBUG ") + defines + " -o obftemp -std=c++1z -lstdc++ -Werror ../official.cpp";
}
std::string buildDebug(std::string defines) {
	return std::string("gcc ") + defines + " -o obftemp -std=c++1z -lstdc++ -Werror ../official.cpp";
}
